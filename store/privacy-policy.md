...
- no servers, and it makes no internet connections
...
Features that read text from an image (Copy OCR Text, Copy Table for Excel, and Smart Redact) use Apple's Vision framework, which runs **entirely on your Mac**. Images aren't sent anywhere to be processed.
