...
**Architecture:**
- `main.js` — Electron main process (CJS); tray icon, global hotkeys (⌘⇧2 region, ⌘⇧3 full, ⌘⇧W window, ⌘⇧5 repeat-region), window lifecycle
...
- `scripts/ocr*.{swift,ps1}` — Vision-framework / Windows.Media.Ocr helpers, asarUnpacked for packaged builds
...
**Captures save to:** `~/Documents/Jack's Picker/` (legacy `~/Documents/Jack's Quick Grab/` is auto-renamed on first launch via `migrateLegacySaveDir`). Annotations live in `.annotations/*.json` + `.flat.png` sidecars.
